...
var chao = {
	 ...
	 */
	colorCodes: [0xff000000, 0xff0000aa, 0xff00aa00, 0xff00aaaa, 0xffaa0000, 0xff800080, 0xff995500, 0xffaaaaaa, 0xff555555, 0xff5555ff, 0xff54ff3f, 0xff55ffff, 0xffff5555, 0xffff55ff, 0xffffff55, 0xffffffff],
...
function ComponentText(font, text, size){
	...
	this.image 			= null;
	this.previousText 	= text;	// last remembered text, stored to detect any changes
	...
	this.changeText = function(text){
		...
		for(var i = 0; i < text.length; ++i){
			...
			if(text[i] == "`"){
				if(this.isNumber(text[i+1]) && this.isNumber(text[i+2])){
					colorStack.push(currentColor);
					newColor = chao.colorCodes[parseInt(text[i+1] + text[i+2], 10)];
					breakChunk = true;
					i += 2;
				} else if(this.isNumber(text[i+1])){
					colorStack.push(currentColor);
					newColor = chao.colorCodes[parseInt(text[i+1], 10)];
					breakChunk = true;
					i += 1;
				} else if(text[i+1] == "`"){
			...
			if(breakChunk){
				if(currentChunk.length > 0){
					chunks.push({text:currentChunk, color:currentColor});
				}
				currentChunk = "";
		...
		for(var i = 0; i < chunks.length; ++i){
			this.rawText += chunks[i].text;
		}

		this.image.context.font = this.size.toFixed() + "px " + this.font.name;
		var textSize = chao.getTextSize(this.image, this.rawText);
		this.image.canvas.width = this.image.width = this.entity.width = textSize.width;
		this.image.canvas.height = this.image.height = this.entity.height = this.size;//textSize.height;
		...
		for(var i = 0; i < chunks.length; ++i){
			chao.drawText(this.image, 
				this.font,
				chunks[i].text,
				currentX, -this.size*.25, // looks about right. gotta check out other fonts
				this.size, 
				chunks[i].color, 
				"left", 
				this.outlineColor, 
